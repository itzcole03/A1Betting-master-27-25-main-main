...
from models.api_models import BettingOpportunity, PerformanceStats
...
try:
    from services.database_service import User, Match, Bet, get_db_session
...
class RealDataService:
    ...
    async def fetch_real_betting_opportunities(self) -> List[BettingOpportunity]:
        ...
        try:
            ...
            logger.info(f"Fetched {len(value_opportunities)} real betting opportunities")
    ...
    def _create_h2h_opportunity(
        self, event: Dict, bookmaker: str, market: Dict, 
    ...
    def _create_totals_opportunity(
        ...
        kelly_fraction = max(0, expected_value / (over_odds - 1)) if over_odds > 1 else 0
    ...
    async def _calculate_value_bets(self, opportunities: List[BettingOpportunity]) -> List[BettingOpportunity]:
                ...
                for opp in event_opps:
                    ...
                    if opp.odds >= best_odds * 0.95:  # Within 5% of best odds
                        ...
                        value_bet = BettingOpportunity(
                            ...
                            recommendation="bet" if enhanced_ev > 0.03 else opp.recommendation
                        )
        ...
        return value_bets[:15]  # Return top 15 opportunities
    ...
    async def fetch_real_performance_stats(self, user_id: Optional[int] = None) -> PerformanceStats:
        ...
        try:
            ...
            # Weekly profit
            weekly_bets = db.query(Bet).filter(
                user_filter,
            ...
            # Monthly profit
            monthly_bets = db.query(Bet).filter(
                user_filter,
            ...
            all_settled_bets = db.query(Bet).filter(
                user_filter,
                Bet.status.in_(["won", "lost"])
            ).all()
            ...
            # Average odds
            avg_odds = sum(bet.odds for bet in all_settled_bets) / total_bets if total_bets > 0 else 0.0
            ...
            logger.info(f"Calculated real performance stats for user {user_id or 'aggregate'}")
                ...
                for projection in data.get("data", []):
                    try:
                        prop = {
                            ...
                            "player": projection.get("attributes", {}).get("player_name", ""),
                            "sport": projection.get("attributes", {}).get("league", ""),
                            "prop_type": projection.get("attributes", {}).get("stat_type", ""),
                            "line": float(projection.get("attributes", {}).get("line_score", 0)),
                            "over_odds": -110,  # Standard PrizePicks odds
                            "under_odds": -110,
                            "confidence": min(0.95, 0.7 + abs(hash(projection.get("id", "")) % 100) / 400),
    ...
    async def _fetch_prizepicks_fallback(self) -> List[Dict[str, Any]]:
        """Fallback PrizePicks props using ESPN player data"""
        try:
            ...
            async with httpx.AsyncClient(timeout=self.http_timeout) as client:
                for sport in sports:
                    try:
                        ...
                        for event in data.get("events", [])[:3]:  # Limit to 3 events per sport
                            for competitor in event.get("competitions", [{}])[0].get("competitors", []):
                                team_name = competitor.get("team", {}).get("displayName", "")
                                
                                # Generate sample props for team players
                                prop_types = ["Points", "Rebounds", "Assists"] if sport == "nba" else ["Passing Yards", "Rushing Yards", "Touchdowns"]
            ...
            ErrorHandler.log_error(e, "generating PrizePicks fallback props")
            return []
