...
async def match_prediction(
    ...
    try:
        logger.info(f"Generating real ML prediction for {req.homeTeam} vs {req.awayTeam}")
        ...
        try:
            ...
            if ml_prediction:
                ...
                if hasattr(req, 'sport') and req.sport and req.sport.lower() in ['soccer', 'football']:
                ...
                if feature_importance:
                    ...
                    key_factors = [f.replace('_', ' ').title() for f, _ in sorted_features[:3]]
                else:
    ...
    try:
        ...
        try:
            ...
            if hasattr(real_ml_service, 'feature_engineer'):
                engineered_features = real_ml_service.feature_engineer.prepare_features(all_features)
        ...
        logger.info(f"Processed {len(feature_names)} features for game {input_data.game_id}")
        return {
            ...
            if performance and 'overall' in performance:
                ...
                for model_name, metrics in performance.items():
                    if model_name != 'overall' and 'accuracy' in metrics:
                        ...
                        feature_importance[f"{model_name}_contribution"] = metrics['accuracy'] / 5
                ...
                trends = {
                    "accuracy_trend": "improving" if model_performance["overall_accuracy"] > 0.8 else "stable",
                    "model_confidence": "high" if model_performance["overall_accuracy"] > 0.85 else "medium",
