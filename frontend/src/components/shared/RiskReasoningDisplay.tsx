...
export function RiskReasoningDisplay({ riskReasoning, className = ''}: RiskReasoningDisplayProps) {
  ...
  const iconForSeverity = (severity: string) => {
    switch (severity) {
      case 'high':
        return (
          <span className='text-red-500 mr-1' title='High Risk' key={215912}>
            ⚠️
          </span>
        );
      case 'medium':
        return (
          <span className='text-yellow-500 mr-1' title='Medium Risk' key={988440}>
            🧠
          </span>
        );
      case 'low':
      default: return (
          <span className='text-green-500 mr-1' title='Low Risk' key={670124}>
            🔍
          </span>
        )}
  };
