...
const UltimateMoneyMaker: React.FC = () => {
  ...
  const scanForOpportunities = async () => {
    setIsScanning(true);
    try {
      const fetchedOpportunities = await ApiService.get<OpportunityCandidate[]>('/api/v1/betting-opportunities');
      ...
      const processedOpportunities = fetchedOpportunities.map((opp: OpportunityCandidate) => ({
        ...opp,
        recommendedStake: Math.min(bankroll * (preferences.riskAppetite / 100), bankroll * opp.kellyFraction * 0.25),
      ...
      processedOpportunities.forEach((opp: OpportunityCandidate) => {
        addOpportunity({
          ...
          prediction: {
            ...
            factors: [], // This might need to be fetched or derived
            timestamp: Date.now(),
          },
    ...
    <div className="space-y-6">
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900 dark:text-white">${bankroll.toFixed(2)}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900 dark:text-white">${totalPotentialReturn.toFixed(2)}</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
          <div className="flex items-center">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-gray-900 dark:text-white">{(averageConfidence * 100).toFixed(1)}%</p>
            </div>
          </div>
        </div>
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
        <div className="p-4 border-b border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white">Betting Opportunities</h2>
          <div className="flex items-center">
            <label htmlFor="autoMode" className="mr-2 text-sm font-medium text-gray-700 dark:text-gray-300">Auto-Scan</label>
        ...
        <div className="divide-y divide-gray-200 dark:divide-gray-700">
          {isScanning && opportunities.length === 0 ? (
            <div className="text-center p-8 text-gray-500">
              ...
              <p className="text-lg">Our AI is scanning the markets for the best opportunities...</p>
            ...
            opportunities.map((opp: OpportunityCandidate) => (
              <div key={opp.id} className="p-4 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors duration-200 flex justify-between items-center">
                <div className="flex-grow">
                  ...
                  <div className="flex items-center space-x-4 mt-2 text-sm">
                    <span>Odds: <span className="font-bold">{opp.currentOdds.toFixed(2)}</span></span>
                    <span>Probability: <span className="font-bold">{(opp.predictedProbability * 100).toFixed(1)}%</span></span>
                    <span>Edge: <span className="font-bold text-green-600">{(opp.valueEdge * 100).toFixed(2)}%</span></span>
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${getRiskColor(opp.riskLevel)}`}>{opp.riskLevel}</span>
                  </div>
                </div>
                <div className="text-right">
                  <p className="font-bold text-lg text-blue-600 dark:text-blue-400">${opp.recommendedStake.toFixed(2)}</p>
      ...
      {selectedOpportunity && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-2xl p-6 w-full max-w-md mx-4">
            ...
            <p className="mb-4 text-gray-600 dark:text-gray-400">{selectedOpportunity.description}</p>

            <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
              <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                <p className="text-gray-500 dark:text-gray-400">Odds</p>
                <p className="font-bold text-lg text-gray-900 dark:text-white">{selectedOpportunity.currentOdds.toFixed(2)}</p>
              </div>
              <div className="bg-gray-50 dark:bg-gray-700/50 p-3 rounded-lg">
                <p className="text-gray-500 dark:text-gray-400">AI Confidence</p>
                <p className="font-bold text-lg text-gray-900 dark:text-white">{(selectedOpportunity.confidence * 100).toFixed(1)}%</p>
              </div>
            </div>

            <div className="mb-4">
              <label htmlFor="stakeAmount" className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Stake Amount</label>
              ...
              <p className="text-xs text-gray-500 mt-1">Recommended: ${selectedOpportunity.recommendedStake.toFixed(2)}. Max: ${selectedOpportunity.maxStake.toFixed(2)}</p>
            </div>
