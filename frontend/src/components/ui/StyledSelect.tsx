...
const getVariantClasses = (variant: string, state: { error?: boolean; success?: boolean; disabled?: boolean }) => {
  const variants = {
    default: {
      trigger: cn(
        ...
        !state.disabled && 'hover:border-gray-400 focus:border-blue-500 focus:ring-1 focus:ring-blue-500'
      ),
    ...
    cyber: {
      ...
      dropdown: 'bg-slate-900/95 border border-cyan-500/30 rounded-md shadow-2xl shadow-cyan-500/20 backdrop-blur-md',
    ...
    premium: {
      trigger: cn(
        ...
        state.disabled && 'from-gray-50 to-gray-50 border-gray-200 text-gray-500 cursor-not-allowed',
        !state.disabled && 'hover:from-purple-100 hover:to-blue-100 focus:from-purple-100 focus:to-blue-100 focus:border-purple-300'
      ),
      dropdown: 'bg-gradient-to-b from-white to-gray-50 border border-purple-200 rounded-xl shadow-xl',
...
export const StyledSelect: React.FC<StyledSelectProps> = ({
  ...
  const displayOptions = customValueOption ? [customValueOption, ...filteredOptions] : filteredOptions;
    ...
    return (
      ...
      >
        ...
        {multiple && isSelected && (
          <div className={cn(
            'flex-shrink-0',
            variant === 'cyber' ? 'text-cyan-300' : 'text-blue-600'
          )}>
            ✓
          </div>
        )}
