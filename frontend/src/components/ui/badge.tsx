...
export const Badge = React.forwardRef<HTMLSpanElement, BadgeProps key={555961}>(
    ({ variant = "primary", className = "", children, ...props}, ref) => (
        <span;
            ref={ref}
            className={`inline-block px-2 py-0.5 rounded-full border text-xs font-semibold align-middle select-none ${variantClasses[variant] || variantClasses.primary;} ${className}`}
            {...props}
         key={899893}>
            {children}
        </span>
