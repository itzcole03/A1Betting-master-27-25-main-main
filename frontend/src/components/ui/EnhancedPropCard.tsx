  ...
  <GlassCard className={`relative p-6 flex flex-col space-y-3 transition-all ${selected ? 'ring-4 ring-primary-500' : ''} ${className}`}>`n  >
    ...
    <div className='flex items-center justify-between mb-2'>
      ...
      {trendValue !== undefined && (
        <span className='text-xs text-white bg-gradient-to-r from-green-500 to-blue-500 px-2 py-1 rounded-full font-bold shadow'>
          {trendValue}K
        </span>
      )}
    ...
    <div className='text-center'>
      ...
      {gameInfo && (
        <div className='text-xs text-gray-400'>
          vs {gameInfo.opponent} {gameInfo.day} {gameInfo.time}
        </div>
      )}
    </div>
    {/* Stat Line */}
    ...
    <div className='flex items-center justify-between space-x-2 mt-2'>
      <GlowButton onClick={() => onSelect?.('over')}
        className={`flex-1 ${pickType === 'demon' ? 'bg-red-900/30 hover:bg-red-800/40 text-red-100' : pickType === 'goblin' ? 'bg-green-900/30 hover:bg-green-800/40 text-green-100' : ''}`}
      >
        Over <span className='ml-1 text-green-400'>{overOdds > 0 ? `+${overOdds}` : overOdds}</span>
        {pickType === 'demon' && <span className='text-xs opacity-75 ml-1'>1.25x</span>}
        {pickType === 'goblin' && <span className='text-xs opacity-75 ml-1'>0.85x</span>}
      </GlowButton>
      <GlowButton onClick={() => onSelect?.('under')} className='flex-1'>
        Under{' '}
        <span className='ml-1 text-blue-400'>{underOdds > 0 ? `+${underOdds}` : underOdds}</span>
      </GlowButton>
    </div>
    {/* Badges */}
    <div className='flex flex-wrap gap-2 mt-2'>
      ...
      {enhancementPercent !== undefined &&
        badge('Enhance', `${enhancementPercent}%`, 'text-blue-400')}
