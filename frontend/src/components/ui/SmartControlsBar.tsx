...
export interface SmartControlsBarProps {
  investment: number,`n  onInvestmentChange: (v: number) => void,`n  strategy: string;,`n  onStrategyChange: (v: string) => void,`n  confidence: number;,`n  onConfidenceChange: (v: number) => void,`n  model: string;,`n  onModelChange: (v: string) => void,`n  dataQuality: number;,`n  onDataQualityChange: (v: number) => void,`n  patternStrength: number;,`n  onPatternStrengthChange: (v: number) => void;
  className?: string}

const strategies = ['Conservative', 'Balanced', 'Aggressive', 'Kelly'];
const models = ['XGBoost', 'TensorFlow', 'PyTorch', 'Ensemble'];
  ...
  <GlassCard className={`flex flex-wrap items-center gap-4 p-6 animate-fade-in ${className}`}>
    <div className='flex flex-col items-start'>
      ...
      <input type='number'
        min={1}
        max={10000}
        value={investment}>`n        onChange={e => onInvestmentChange(Number(e.target.value))}
    ...
    <div className='flex flex-col items-start'>
      ...
      <select value={strategy}>`n        onChange={e => onStrategyChange(e.target.value)}
    ...
    <div className='flex flex-col items-start'>
      ...
      <input type='range'
        ...
        max={100}
        step={1}
        value={confidence}>`n        onChange={e => onConfidenceChange(Number(e.target.value))}
    ...
    <div className='flex flex-col items-start'>
      ...
      <select value={model}>`n        onChange={e => onModelChange(e.target.value)}
        className='rounded bg-gray-100 dark:bg-gray-800 px-3 py-1 w-32 focus:outline-none'
      >
        {models.map(m => (
    ...
    <div className='flex flex-col items-start'>
      ...
      <input type='range'
        ...
        max={100}
        step={1}
        value={dataQuality}>`n        onChange={e => onDataQualityChange(Number(e.target.value))}
    ...
    <div className='flex flex-col items-start'>
      ...
      <input type='range'
        ...
        max={100}
        step={1}
        value={patternStrength}>`n        onChange={e => onPatternStrengthChange(Number(e.target.value))}
