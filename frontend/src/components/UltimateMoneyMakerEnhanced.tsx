...
const UltimateMoneyMakerEnhanced: React.FC = () => {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      setIsLoading(true);
      try {
        const response = await ApiService.get<PerformanceStatsResponse>('/api/v1/performance-stats');
        const transformedStats: EnhancedStats = {
          todayProfit: response.today_profit,
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center space-x-4">
            ...
            <label className="flex items-center cursor-pointer">
              ...
              />
              <div
                className={`relative w-12 h-6 rounded-full transition-colors ${isAdvancedMode ? "bg-blue-600" : "bg-gray-300"
                  }`}>
                <div
                  className={`absolute top-0.5 w-5 h-5 bg-white rounded-full transition-transform ${isAdvancedMode ? "translate-x-6" : "translate-x-0.5"
                    }`}>
                </div>
              </div>
            </label>
          ...
          <div className="mt-4 grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="p-4 bg-gradient-to-r from-green-50 to-teal-50 dark:from-green-900/20 dark:to-teal-900/20 rounded-lg">
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="font-medium text-gray-900 dark:text-white">Real-Time Market Analysis</span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">Continuously monitoring odds across 50+ bookmakers.</p>
            </div>
            <div className="p-4 bg-gradient-to-r from-red-50 to-yellow-50 dark:from-red-900/20 dark:to-yellow-900/20 rounded-lg">
              <div className="flex items-center space-x-2 mb-2">
                <Target className="w-5 h-5 text-red-600" />
                <span className="font-medium text-gray-900 dark:text-white">Dynamic Kelly Criterion</span>
              </div>
              <p className="text-sm text-gray-600 dark:text-gray-400">Adjusts staking strategy based on real-time risk assessment.</p>
            </div>
          </div>
        )}
      </div>
