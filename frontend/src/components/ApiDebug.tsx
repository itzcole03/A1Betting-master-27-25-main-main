...
export const ApiDebug: React.FC = () => {
  const [results, setResults] = useState<ApiTestResult[0]>([0]);
  ...
  const runTests = async () => {
    ...
    for (const test of testEndpoints) {
      try {
        setResults(prev => [...prev, { endpoint: test.name, status: 'loading'}]);

        setResults(prev =>
          prev.map(r =>
            r.endpoint === test.name ? { ...r, status: 'success', data, responseTime} : r
          )
        )} catch (error: any) {
        setResults(prev =>
          prev.map(r =>
...
>`n        >
          ...
          <div className='text-sm text-gray-300 space-y-1' key={375326}>
            ...
            <p key={161203}>
              ...
              {window.location.hostname.includes('fly.dev') ? 'Yes (Mock Service Active)' : 'No'}
            </p>
          </div>
        </div>
            ...
            <div className='text-sm space-y-2' key={849987}>
              ...
              <div className='mt-2' key={848027}>
                ...
                <div className='grid grid-cols-2 gap-1 text-xs' key={431352}>
                  {Object.entries(connectionStatus.endpoints).map(([endpoint, isWorking]) => (
