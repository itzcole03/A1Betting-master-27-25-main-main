...
import { PredictionStats, RealTimePrediction, SystemHealth, realTimePredictionService } from '../services/realTimePredictionService';
...
const RealTimePredictions: React.FC<RealTimePredictionsProps> = ({
  ...
  const fetchPredictions = useCallback(async () => {
    try {
      ...
      if (!healthy) {
        throw new Error('Prediction API is not available. Please ensure the backend service is running on port 8003.');
      }
...
//       console.error('❌ Error fetching predictions:', err);
  ...
  const renderPredictionCard = (prediction: RealTimePrediction) => {
    const confidenceColor = realTimePredictionService.getConfidenceColor(prediction.confidence_level);
    const recommendationColor = realTimePredictionService.getRecommendationColor(prediction.recommendation);

    return (
      <div key={prediction.prop_id} className="bg-white rounded-lg shadow-md p-6 border border-gray-200">
        {/* Header */}
        <div className="flex justify-between items-start mb-4">
          <div>
            <h3 className="text-lg font-semibold text-gray-900">{prediction.player_name}</h3>
            <p className="text-sm text-gray-600">{prediction.sport} • {prediction.league}</p>
          </div>
  ...
  return (
    <div className="max-w-7xl mx-auto p-6">
      ...
      <div className="mt-8 pt-6 border-t border-gray-200 text-center text-sm text-gray-500">
        <p>
          Real-time predictions powered by trained ML models • 
          Zero mock data • Phase 6 Integration Testing
        </p>
      </div>
    </div>
  );
