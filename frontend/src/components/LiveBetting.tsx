...
export const LiveBetting: React.FC = () => {
  ...
  const generateLiveGames = (): LiveGame[] => {
    ...
    for (let i = 0; i < 8; i++) {
      ...
      const period = sport === 'NBA' || sport === 'NFL' 
      ...
      games.push({
        ...
        period,
        timeRemaining: `${Math.floor(Math.random() * 12)}:${Math.floor(Math.random() * 59).toString().padStart(2, '0')}`,
              ...
              >
                ...
                >
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      <span className={`font-semibold ${game.homeScore > game.awayScore ? 'text-green-400' : 'text-white'}`}>
                        {game.homeTeam}
                      </span>
                      <span className="text-2xl font-bold text-white">{game.homeScore}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className={`font-semibold ${game.awayScore > game.homeScore ? 'text-green-400' : 'text-white'}`}>
                        {game.awayTeam}
                      </span>
                  ...
                  <div className="space-y-2 mb-4">
                    {betSlip.map((bet) => (
                      <div key={bet.id} className="flex items-center justify-between text-sm">
                        <span className="text-gray-300">{bet.type} • {bet.odds}</span>
                        <button
