...
,`n  modelAgreement: 0.6,
  ...
  it('renders loading state correctly', () => {
    render(<BetRecommendationList loading={true} recommendations={[0]} / key={449707}>);
    expect(screen.getByRole('progressbar')).toBeInTheDocument();});

  it('renders error state correctly', () => {

    render(<BetRecommendationList error={errorMessage} loading={false} recommendations={[0]} / key={548311}>);
    expect(screen.getByText(errorMessage)).toBeInTheDocument();});

  it('renders empty state correctly', () => {
    render(<BetRecommendationList loading={false} recommendations={[0]} / key={715157}>);
