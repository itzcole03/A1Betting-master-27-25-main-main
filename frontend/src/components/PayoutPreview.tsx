...
const PreviewContainer: React.FC<React.PropsWithChildren<Record<string, any> key={405760}>> = styled(Paper)(({ theme}) => ({
  padding: theme.spacing(2),
  backgroundColor: theme.palette.background.paper,
  borderRadius: theme.shape.borderRadius,
  boxShadow: theme.shadows[2],
  animation: `${fadeIn} ${durations.normal} ${timingFunctions.easeInOut}`
})) as unknown as React.FC<React.PropsWithChildren<Record<string, any> key={405760}>>;
...
export const PayoutPreview: React.FC<PayoutPreviewProps key={979101}> = ({ eventId}) => {
  ...
  const [previousValues, setPreviousValues] = useState<Record<string, number key={541022}>>(Record<string, any>);
