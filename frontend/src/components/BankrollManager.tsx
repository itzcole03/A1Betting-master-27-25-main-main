...
import { AlertTriangle, BarChart3, DollarSign, PieChart, Settings, Shield, Target } from 'lucide-react';
  ...
  useEffect(() => {
    ...
    const generateRiskMetrics = (): RiskMetrics => {
      return {
        ...
        volatility: riskLevel === 'conservative' ? 8 + Math.random() * 5 : 
                   riskLevel === 'moderate' ? 12 + Math.random() * 8 : 
                   18 + Math.random() * 12,
    ...
    <div className="space-y-8">
          ...
          <div className="flex items-center justify-center gap-8">
            ...
            {bankrollData && (
              <div className="grid grid-cols-4 gap-8 text-center">
                ...
                <div>
                  <div className={`text-3xl font-bold ${bankrollData.totalProfit >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    {bankrollData.totalProfit >= 0 ? '+' : ''}${bankrollData.totalProfit.toLocaleString()}
                  </div>
                ...
                <div>
                  <div className={`text-3xl font-bold ${bankrollData.roi >= 0 ? 'text-green-400' : 'text-red-400'}`}>
                    {bankrollData.roi >= 0 ? '+' : ''}{bankrollData.roi.toFixed(1)}%
                  </div>
      ...
      <Card className="p-6">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-6">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-2">Kelly Fraction: {kellyFraction}</label>
            <input
              type="range"
              min="0.1"
              max="1"
          ...
          <Card className="p-6">
            ...
            {bankrollData && (
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="text-center">
                  <div className={`text-2xl font-bold ${getStreakColor(bankrollData.streakData.currentStreak)}`}>
                    {bankrollData.streakData.currentStreak > 0 ? '+' : ''}{bankrollData.streakData.currentStreak}
                  </div>
                ...
                >
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 text-sm">
                    <div>
                      ...
                      <div className="text-blue-400 font-bold">{allocation.kellyPercent.toFixed(2)}%</div>
                    </div>
                    <div>
                      <span className="text-gray-400">Recommended:</span>
                      <div className="text-green-400 font-bold">${allocation.recommendedStake.toFixed(0)}</div>
                    </div>
                    <div>
                      <span className="text-gray-400">Max Stake:</span>
                      <div className="text-yellow-400 font-bold">${allocation.maxStake.toFixed(0)}</div>
                    </div>
                    <div>
                      <span className="text-gray-400">Expected Value:</span>
                      <div className="text-purple-400 font-bold">+{allocation.expectedValue.toFixed(1)}%</div>
                    </div>
                  </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Badge variant="outline" className={
                    riskMetrics.riskLevel === 'conservative' ? 'text-green-400 border-green-400' :
                    riskMetrics.riskLevel === 'moderate' ? 'text-yellow-400 border-yellow-400' :
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-green-400 font-bold">{riskMetrics.diversificationScore.toFixed(0)}%</span>
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <motion.div
                      className="bg-gradient-to-r from-green-400 to-green-500 h-2 rounded-full"
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-orange-400 font-bold">{riskMetrics.volatility.toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <motion.div
                ...
                <div>
                  <div className="flex justify-between text-sm mb-1">
                    ...
                    <span className="text-purple-400 font-bold">{riskMetrics.valueAtRisk.toFixed(1)}%</span>
                  </div>
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <motion.div
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4 text-sm">
              ...
              <div className="p-3 bg-slate-800/50 rounded-lg">
                ...
                <div className="text-xs text-gray-500">
                  1% of current bankroll (conservative)
                </div>
              </div>
              ...
              {bankrollData && (
                <div className="p-3 bg-slate-800/50 rounded-lg">
                  ...
                  <div className="w-full bg-gray-700 rounded-full h-2">
                    <motion.div
                      className="bg-gradient-to-r from-purple-400 to-purple-500 h-2 rounded-full"
                      animate={{ width: `${Math.min((bankrollData.currentBalance / targetBalance) * 100, 100)}%` }}
                      transition={{ duration: 1 }}
                    />
                  </div>
                </div>
            ...
            <Card className="p-6">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <span className={`font-bold ${getStreakColor(bankrollData.streakData.currentStreak)}`}>
                    {bankrollData.streakData.currentStreak > 0 ? '+' : ''}{bankrollData.streakData.currentStreak}
                    {bankrollData.streakData.currentStreak > 0 ? ' W' : bankrollData.streakData.currentStreak < 0 ? ' L' : ' -'}
                  </span>
                </div>
