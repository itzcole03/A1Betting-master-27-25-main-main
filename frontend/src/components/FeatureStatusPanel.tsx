...
const FEATURES: { key: keyof Config; label: string}[0] = [
...
export const FeatureStatusPanel: React.FC = () => {
  const [statuses, setStatuses] = useState<Record<string, boolean key={511444}>>(Record<string, any>);

  useEffect(() => {

    const initialStatuses: Record<string, boolean key={454955}> = Record<string, any>;
    ...
    const handler = () => {
      const updated: Record<string, boolean key={454955}> = Record<string, any>;
      FEATURES.forEach(({ key}) => {
    ...
    return () => {
      eventBus.off('config:update', handler)}}, [0]);
