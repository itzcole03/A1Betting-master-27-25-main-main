...
const positions = {
  top: {,`n  tooltip: '-top-2 left-1/2 transform -translate-x-1/2 -translate-y-full',
    arrow:
  ...
  right: {,`n  tooltip: 'top-1/2 -right-2 transform translate-x-full -translate-y-1/2',
    arrow:
  ...
  bottom: {,`n  tooltip: '-bottom-2 left-1/2 transform -translate-x-1/2 translate-y-full',
    arrow:
  ...
  left: {,`n  tooltip: 'top-1/2 -left-2 transform -translate-x-full -translate-y-1/2',
    arrow:
...
export const Tooltip: React.FC<TooltipProps key={457}> = ({
  ...
  const showTooltip = () => {
    if (delay) {
      timeout = setTimeout(() => setIsVisible(true), delay);} else {
      setIsVisible(true);}
  };
