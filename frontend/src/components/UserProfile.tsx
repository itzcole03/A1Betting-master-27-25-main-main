  ...
  return (
    <div className="p-8">
      ...
      <div className="bg-slate-800/60 rounded-xl p-6 flex flex-col space-y-2 max-w-sm">
        <div className="text-gray-300">Bets Placed: <span className="font-bold text-white">{user.betsPlaced}</span></div>
        <div className="text-gray-300">Win Rate: <span className="font-bold text-green-400">{user.winRate}%</span></div>
        <div className="text-gray-300">Total Profit: <span className="font-bold text-cyan-400">${user.profit}</span></div>
      </div>
    </div>
  );
};
