...
import { Activity, AlertTriangle, Clock, Heart, Search, Target, TrendingDown, User } from 'lucide-react';
...
export const InjuryTracker: React.FC = () => {
  ...
  useEffect(() => {
    const generateInjuryReports = (): InjuryReport[] => {
      ...
      const injuries = [
        ...
        'Concussion', 'Back Spasms', 'Groin Strain', 'Wrist Injury',
        'Hip Injury', 'Calf Strain', 'Neck Injury', 'Foot Injury'
      ];
      
      const bodyParts = [
        'Ankle', 'Knee', 'Hamstring', 'Shoulder', 'Head', 'Back',
        'Groin', 'Wrist', 'Hip', 'Calf', 'Neck', 'Foot'
      ];

      return Array.from({ length: 20 }, (_, index) => {
        const sport = sports[Math.floor(Math.random() * sports.length)];
        const team = teams[sport][Math.floor(Math.random() * teams[sport].length)];
        const severity = Math.random() > 0.7 ? 'severe' : Math.random() > 0.4 ? 'moderate' : 'minor';
        ...
        return {
          ...
          expectedReturn: severity === 'severe' ? '4-6 weeks' : severity === 'moderate' ? '1-2 weeks' : '3-5 days',
          impact: {
            team: severity === 'severe' ? 80 + Math.random() * 20 : severity === 'moderate' ? 50 + Math.random() * 30 : 20 + Math.random() * 30,
            fantasy: severity === 'severe' ? 90 + Math.random() * 10 : severity === 'moderate' ? 60 + Math.random() * 30 : 30 + Math.random() * 40,
            betting: severity === 'severe' ? 70 + Math.random() * 30 : severity === 'moderate' ? 40 + Math.random() * 40 : 15 + Math.random() * 35
          },
          timeline: `${Math.floor(Math.random() * 12) + 1}h ago`,
          reportedDate: `${Math.floor(Math.random() * 7) + 1} days ago`,
          source: ['Team Report', 'Beat Reporter', 'Medical Staff', 'Player Statement'][Math.floor(Math.random() * 4)],
    ...
    const generateTeamImpacts = (injuryData: InjuryReport[]): TeamImpact[] => {
      ...
      return Array.from(teamMap.entries()).map(([key, teamInjuries]) => {
        const [team, sport] = key.split('-');
        const keyPlayerInjuries = teamInjuries.filter(i => 
        ...
        return {
          ...
          keyPlayerInjuries,
          projectedImpact: Math.min(100, teamInjuries.reduce((sum, i) => sum + i.impact.team, 0) / 3),
  ...
  const filteredInjuries = injuries.filter(injury => {
    ...
    if (searchQuery && !injury.player.toLowerCase().includes(searchQuery.toLowerCase()) && 
        !injury.team.toLowerCase().includes(searchQuery.toLowerCase())) return false;
    return true;
  });

  const getSeverityColor = (severity: string) => {
    switch (severity) {
      case 'critical': return 'text-red-600 border-red-600 bg-red-900/20';
      case 'severe': return 'text-red-400 border-red-400 bg-red-900/10';
      case 'moderate': return 'text-yellow-400 border-yellow-400 bg-yellow-900/10';
      case 'minor': return 'text-green-400 border-green-400 bg-green-900/10';
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      case 'out': case 'ir': return 'text-red-400 border-red-400';
      case 'doubtful': return 'text-orange-400 border-orange-400';
      case 'questionable': return 'text-yellow-400 border-yellow-400';
  ...
  const getSeverityIcon = (severity: string) => {
    switch (severity) {
      case 'critical': case 'severe': return <AlertTriangle className="w-4 h-4" />;
      case 'moderate': return <TrendingDown className="w-4 h-4" />;
      case 'minor': return <Activity className="w-4 h-4" />;
                    ...
                    <div className="flex items-start justify-between mb-4">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-gray-400 text-sm">{injury.team} • {injury.position}</p>
                        </div>
                      </div>
                        ...
                        <div className="space-y-2">
                          <div>
                            <div className="flex justify-between text-sm mb-1">
                              ...
                              <span className="text-orange-400 font-bold">{injury.impact.team.toFixed(0)}%</span>
                            </div>
                          ...
                          <div>
                            <div className="flex justify-between text-sm mb-1">
                              ...
                              <span className="text-purple-400 font-bold">{injury.impact.fantasy.toFixed(0)}%</span>
                            </div>
                            <div className="w-full bg-gray-700 rounded-full h-2">
                              <motion.div
                          ...
                          <div>
                            <div className="flex justify-between text-sm mb-1">
                              ...
                              <span className="text-blue-400 font-bold">{injury.impact.betting.toFixed(0)}%</span>
                            </div>
                            <div className="w-full bg-gray-700 rounded-full h-2">
                              <motion.div
