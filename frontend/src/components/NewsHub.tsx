...
export const NewsHub: React.FC = () => {
  ...
  const [sortBy, setSortBy] = useState<'timestamp' | 'impact' | 'credibility' | 'engagement'>('timestamp');
  ...
  const generateNewsArticles = (): NewsArticle[] => {
    ...
    const sources = ['ESPN', 'The Athletic', 'Bleacher Report', 'Yahoo Sports', 'CBS Sports', 'NBC Sports'];
    
    const sampleNews = [
      {
        title: "Star Player Questionable for Tonight's Game",
        summary: "Team officials report that the All-Star player is dealing with a minor injury that could affect availability for the crucial matchup tonight."
      },
      {
        title: "Trade Deadline Approaching: Teams Making Final Moves",
        summary: "Multiple teams are reportedly in talks for last-minute trades that could reshape the playoff picture."
      },
      {
        title: "Weather Forecast Could Impact Outdoor Games",
        summary: "Meteorologists predict adverse conditions that may affect several games scheduled for this weekend."
      },
      {
        title: "Record-Breaking Performance Lights Up Social Media",
        summary: "Fans and analysts are buzzing about a historic achievement that happened in last night's game."
      },
      {
        title: "Coaching Change Rumored After Poor Start",
        summary: "Sources close to the organization suggest management is considering major changes following recent struggles."
      },
      {
        title: "Rookie Sensation Continues Impressive Campaign",
        summary: "The first-year player has exceeded all expectations and is now in the conversation for major awards."
      },
      {
        title: "Vegas Lines Moving Significantly on Upcoming Matchup",
        summary: "Sharp money appears to be influencing odds on what was expected to be a straightforward game."
      },
      {
        title: "International Player Visa Issues Cause Uncertainty",
        summary: "Administrative complications could sideline key international talent for several games."
      }
    ];
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'breaking': return 'text-red-400 border-red-400';
      case 'injury': return 'text-orange-400 border-orange-400';
      case 'trade': return 'text-blue-400 border-blue-400';
      case 'analysis': return 'text-purple-400 border-purple-400';
      case 'prediction': return 'text-green-400 border-green-400';
              ...
              {trendingTopics.map((topic, index) => (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <Badge variant="outline" className={`${topic.growth > 0 ? 'text-green-400 border-green-400' : 'text-red-400 border-red-400'}`}>
