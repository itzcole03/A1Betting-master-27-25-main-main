                  ...
                  <div>
                    ...
                    <div className="flex items-center space-x-2 mt-2">
                      <span className={`px-3 py-1 rounded-full text-xs font-medium ${
                        userProfile.subscription.plan === 'elite' ? 'bg-purple-500/20 text-purple-400' :
                        userProfile.subscription.plan === 'pro' ? 'bg-cyan-500/20 text-cyan-400' :
              ...
              <div className="flex items-start space-x-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-yellow-300/80 mt-1">
                    Your privacy is important to us. These settings control how your data is collected, used, and shared.
                  </p>
                </div>
              </div>
            </div>
                ...
                <div key={key} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                  <div>
                    ...
                    <p className="text-sm text-gray-400 mt-1">
                      ...
                      {key === 'dataCollection' && 'Allow collection of usage data for platform improvement'}
                      {key === 'analyticsTracking' && 'Enable analytics tracking for personalized experience'}
                      {key === 'marketingCommunications' && 'Receive marketing communications from partners'}
                      {key === 'thirdPartySharing' && 'Allow sharing data with trusted third-party services'}
                    </p>
                  </div>
                  {key === 'profileVisibility' ? (
                    ...
                    <ToggleSwitch
                      ...
                      onChange={(enabled) => setPrivacySettings(prev => ({
                        ...prev,
                        [key]: enabled
                      }))}
                    />
        ...
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Default Sport</label>
                <select
            ...
            <div className="space-y-4">
              {[
                ...
                { key: 'soundEffects', label: 'Sound Effects', desc: 'Audio feedback for actions and alerts' },
                { key: 'autoRefresh', label: 'Auto Refresh', desc: 'Automatically refresh data at set intervals' },
                { key: 'compactMode', label: 'Compact Mode', desc: 'More dense layout with smaller components' },
                { key: 'expertMode', label: 'Expert Mode', desc: 'Advanced features and detailed metrics' },
              ].map(({ key, label, desc }) => (
                <div key={key} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                  <div>
                  ...
                  <ToggleSwitch
                    ...
                    onChange={(enabled) => setPlatformSettings(prev => ({
                      ...prev,
                      [key]: enabled
                    }))}
                  />
      ...
      case 'betting':
        return (
          <div className="space-y-6">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Default Bankroll ($)</label>
                <input
                  type="number"
                  min="0"
                  value={bettingSettings.defaultBankroll}
                  onChange={(e) => setBettingSettings(prev => ({
                    ...prev,
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Kelly Fraction</label>
                <input
                  type="number"
                  min="0"
                  max="1"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-300 mb-2">Arbitrage Threshold (%)</label>
                <input
                  type="number"
                  min="0"
                  max="50"
            ...
            <div className="space-y-4">
              {[
                { key: 'autoStaking', label: 'Auto Staking', desc: 'Automatically calculate bet sizes using Kelly Criterion' },
                { key: 'confirmBets', label: 'Confirm Bets', desc: 'Require confirmation before placing bets' },
                { key: 'trackingEnabled', label: 'Bet Tracking', desc: 'Track all bets and calculate performance metrics' },
              ].map(({ key, label, desc }) => (
                <div key={key} className="flex items-center justify-between p-4 bg-slate-800/50 rounded-lg">
                  <div>
                    <span className="text-white font-medium">{label}</span>
                  ...
                  <ToggleSwitch
                    ...
                    onChange={(enabled) => setBettingSettings(prev => ({
                      ...prev,
                      [key]: enabled
                    }))}
                  />
          ...
          <div className="space-y-6">
                  ...
                  <div key={provider} className="p-4 bg-slate-800/50 rounded-lg">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      <ToggleSwitch
                        ...
                        onChange={(enabled) => setAPISettings(prev => ({
                          ...prev,
                          providers: {
                            ...prev.providers,
            ...
            <div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-2">Max Cache Size (MB)</label>
                  <input
                    type="number"
          ...
          >
            <div className="flex items-center space-x-3 mb-6">
              {tabs.find(tab => tab.id === activeTab)?.icon && (
                <tabs.find(tab => tab.id === activeTab)!.icon className="w-6 h-6 text-cyan-400" />
              )}
