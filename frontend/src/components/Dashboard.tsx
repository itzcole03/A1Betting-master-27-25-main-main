...
const Dashboard: React.FC<DashboardProps> = ({
  ...
  const actualBets = recentBets || (realtimeData?.upcomingOpportunities?.slice(0, 4).map((opportunity: any, index: number) => ({
    id: index + 1,
  ...
  const actualPredictions = livePredictions || (realtimeData?.upcomingOpportunities?.slice(0, 4).map((opportunity: any, index: number) => ({
    id: index + 1,
    match: opportunity.game || 'Live Match',
