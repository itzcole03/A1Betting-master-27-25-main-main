...
export const SHAPAnalysis: React.FC = () => {
  ...
  const generatePredictions = (): Prediction[] => {
    ...
    return games.map((g, index) => ({
      ...
      actualOutcome: Math.random() > 0.3 ? undefined : Math.random() > 0.5 ? 'Correct' : 'Incorrect'
    }));
  };
  ...
  const generateInsights = (prediction: Prediction): ExplanationInsight[] => {
    ...
    // Risk factor
    const riskFactor = shapValues.find(s => s.impact === 'negative' && Math.abs(s.contribution) > 3);
    if (riskFactor) {
      insights.push({
        type: 'risk_factor',
    ...
    // Confidence booster
    const confidenceBooster = shapValues.find(s => s.impact === 'positive' && Math.abs(s.contribution) > 4);
    ...
    // Unexpected factor
    const unexpected = shapValues.find(s => s.category === 'external' && Math.abs(s.contribution) > 2);
    if (unexpected) {
      insights.push({
        type: 'unexpected',
  ...
  const getCategoryColor = (category: string) => {
    switch (category) {
      case 'player': return 'text-blue-400 border-blue-400';
      case 'team': return 'text-green-400 border-green-400';
      case 'game': return 'text-purple-400 border-purple-400';
      case 'external': return 'text-orange-400 border-orange-400';
      case 'historical': return 'text-cyan-400 border-cyan-400';
  ...
  const getInsightTypeColor = (type: string) => {
    switch (type) {
      case 'key_driver': return 'border-yellow-500/30 bg-yellow-900/10';
      case 'risk_factor': return 'border-red-500/30 bg-red-900/10';
      case 'confidence_booster': return 'border-green-500/30 bg-green-900/10';
      case 'unexpected': return 'border-purple-500/30 bg-purple-900/10';
          ...
          {selectedPrediction ? (
            <div className="space-y-6">
              ...
              <Card className="p-6">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    ...
                    <p className="text-green-400 text-lg font-bold mt-1">{selectedPrediction.prediction}</p>
                  </div>
                ...
                <div className="flex items-center gap-4 text-sm">
                  ...
                  <span className="text-gray-400">Base Value: {selectedPrediction.baseValue.toFixed(3)}</span>
              ...
              <Card className="p-6">
                ...
                <div className="space-y-3">
                  {selectedPrediction.shapValues.slice(0, analysisDepth === 'basic' ? 6 : analysisDepth === 'detailed' ? 10 : 12).map((shap, index) => (
                    <motion.div
                      key={index}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                    ...
                    >
                      ...
                      <div className="relative">
                        ...
                        <div className="text-xs text-gray-400 mt-1">
                          Value: {shap.value.toFixed(1)} | Impact: {Math.abs(shap.contribution).toFixed(2)}
                        </div>
                      </div>
                    </motion.div>
                  ))}
                  ...
                  <div className="border-t border-gray-700 pt-4">
                    <p className="text-gray-300 text-sm leading-relaxed">
                      ...
                      <span className="text-yellow-400 font-bold">
                        {selectedPrediction.shapValues.reduce((sum, s) => sum + s.contribution, 0).toFixed(2)}
                      </span>
