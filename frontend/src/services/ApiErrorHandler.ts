...
export class ApiErrorHandler {
  static handleError<T>(error: any, operation: string, defaultValue: T): T {
    ...
    if (isNetworkError) {
      ...
      consoleManager.logBackendOffline();} else {
      ...
      if (error.response?.status === 404) {
        ...
        technicalMessage = 'API endpoint not available';} else if (error.response?.status === 500) {
        userMessage = 'Server error occurred';
        technicalMessage = 'Internal server error';} else if (error.response?.status === 401) {
