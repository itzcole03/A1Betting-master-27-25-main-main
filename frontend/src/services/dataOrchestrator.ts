...
predictionService.subscribeToPredictions(async (prediction: PredictionResult) => {
  try {
    ...
    const shap = await ShapExplainerService.explainPrediction(
      prediction.metadata?.modelVersion || Record<string, any>,
      prediction.metadata?.features || Record<string, any>
    );
...
//       analytics
    ...
    logLiveData(`Analytics pipeline complete for prediction ${prediction.id}`);} catch (err) {
