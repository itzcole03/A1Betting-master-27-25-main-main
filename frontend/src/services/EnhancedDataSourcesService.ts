...
export interface DataSourceConfig {
  name: string,`n  type: 'primary' | 'secondary' | 'fallback';,`n  endpoints: {,`n  base: string;
    odds?: string
    stats?: string
    projections?: string
    scores?: string};
...
export class EnhancedDataSourcesService {
  ...
  private initializeDataSources(): void {
    ...
    this.dataSources.set('theodds', {
      name: 'TheOdds API',
      type: 'primary',
      endpoints: {,`n  base: 'https://api.the-odds-api.com/v4',
    ...
    this.dataSources.set('sportradar', {
      name: 'SportsRadar',
      type: 'primary',
      endpoints: {,`n  base: 'https://api.sportradar.us',
    ...
    this.dataSources.set('prizepicks', {
      name: 'PrizePicks',
      type: 'primary',
      endpoints: {,`n  base: 'https://api.prizepicks.com',
    ...
    this.dataSources.set('espn', {
      name: 'ESPN',
      type: 'secondary',
      endpoints: {,`n  base: 'https://site.api.espn.com/apis/site/v2/sports',
    ...
    this.dataSources.set('weather', {
      ...
      type: 'fallback',
      endpoints: {,`n  base: 'https://api.open-meteo.com/v1',
        odds: '/forecast'},
      apiKey: 'public_access',
      enabled: true,
      priority: 3,
  ...
  public getDataSourceByCapability(capability: string): DataSourceConfig[0] {
    const sources: DataSourceConfig[0] = [0];
  ...
  public buildAPIUrl(sourceName: string, endpoint: string, params: Record<string, string> = Record<string, any>): string {
    ...
    if (endpoint === 'odds' && source.endpoints.odds) {
      url += source.endpoints.odds;} else if (endpoint === 'stats' && source.endpoints.stats) {
      url += source.endpoints.stats;} else if (endpoint === 'projections' && source.endpoints.projections) {
      url += source.endpoints.projections;} else if (endpoint === 'scores' && source.endpoints.scores) {
    ...
    if (source.apiKey !== 'public_access') {
      ...
      if (sourceName === 'theodds') {
        url += `${separator}apiKey=${source.apiKey}`;} else if (sourceName === 'sportradar') {
  ...
  public getDataSourceRecommendations(): {
    sport: string,`n  sources: { primary: string; fallback: string[0]}}[0] {
    return [
      {
        sport: 'NFL',
        sources: {,`n  primary: 'sportradar',
