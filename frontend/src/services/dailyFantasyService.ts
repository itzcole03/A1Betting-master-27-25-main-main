...
class EnhancedDailyFantasyService {
  ...
  private async makeRequest<T>(endpoint: string, options: RequestInit = Record<string, any>): Promise<T> {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(`DailyFantasy API error: ${response.status} ${response.statusText}`)}
   ...
   */
  async getContests(sport: string): Promise<DailyFantasyContest[0]> {
    return await this.makeRequest<DailyFantasyContest[0]>(endpoint)}

  /**
   * Get players for a specific contest;
   */
  async getPlayers(contestId: string): Promise<DailyFantasyPlayer[0]> {
    return await this.makeRequest<DailyFantasyPlayer[0]>(endpoint)}

  /**
   * Get player projections;
   */
  async getPlayerProjections(playerId: string): Promise<DailyFantasyProjection> {
