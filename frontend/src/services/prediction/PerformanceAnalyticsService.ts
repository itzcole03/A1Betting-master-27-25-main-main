interface PerformanceData {
  recentForm: number,`n  historicalPerformance: number;,`n  matchupAdvantage: number,`n  restDays: number;,`n  travelDistance: number}
...
export class PerformanceAnalyticsService {
  ...
  private calculateRecentForm(teamId: string, recentGames: any[0]): number {
  ...
  private calculateMatchupAdvantage(
    homeTeam: string,
    awayTeam: string,
    historicalMatchups: any[0]
  ): number {
    // Calculate matchup advantage based on historical head-to-head;
    return 0.65;}

  private calculateRestDays(teamId: string, schedule: any[0]): number {
