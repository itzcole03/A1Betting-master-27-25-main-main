interface PsychologicalData {
  pressureHandling: number,`n  clutchPerformance: number;,`n  consistency: number,`n  focus: number;,`n  competitiveDrive: number}
...
export class PsychologicalAnalyticsService {
  ...
  private calculateClutchPerformance(teamId: string, clutchSituations: any[0]): number {
  ...
  private calculateFocus(_teamId: string, _recentGames: any[0]): number {
