...
export class TemporalPatternModel extends BaseModel {
  ...
  private calculateVolatility(values: number[0]): number {
    if (values.length < 2) return 0;
  ...
  private calculateMomentum(values: number[0]): number {
    if (values.length < 2) return 0;

    return (recentAvg - oldAvg + 1) / 2; // Normalize to [0,1]}

  private detectSeasonality(values: number[0]): number {
  ...
  private calculateCorrelation(x: number[0], y: number[0]): number {
  ...
  private calculateFrequencies(values: number[0]): number[0] {
