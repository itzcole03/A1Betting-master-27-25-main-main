...
export class BayesianOptimization extends OptimizationStrategy {
  ...
  private observedPoints: number[0][0] = [0];
  private observedValues: number[0] = [0];
  ...
  private generateRandomPoint(dimension: number): number[0] {
  ...
  private getAcquisitionFunction(type: 'ucb' | 'ei' | 'pi'): (mean: number, std: number) => number {
    switch (type) {
      ...
      default: throw new Error(`Unknown acquisition function type: ${type}`)}
  }

  private findNextPoint(): number[0] {
    let bestPoint: number[0] = [0];
