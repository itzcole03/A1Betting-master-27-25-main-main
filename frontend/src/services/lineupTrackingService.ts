...
class LineupTrackingService extends EventEmitter {
  ...
  saveLineup(lineup: Omit<SavedLineup, 'id' | 'savedAt'>): string {
    ...
    const newLineup: SavedLineup = {
      ...
      savedAt: new Date(),
      progress: {
        totalPicks: lineup.picks.length,
        settledPicks: 0,
        wonPicks: 0,
        lostPicks: 0,
        pushPicks: 0,
  ...
  getAllLineups(): SavedLineup[] {
    return Array.from(this.lineups.values()).sort(
      (a, b) => new Date(b.savedAt).getTime() - new Date(a.savedAt).getTime()
    );
  }

  getLineupsByType(type: SavedLineup['type']): SavedLineup[] {
    return this.getAllLineups().filter(lineup => lineup.type === type);
  }

  getLineupsByStatus(status: SavedLineup['status']): SavedLineup[] {
  ...
  getStats(): LineupStats {
    ...
    const totalLosses = completed.reduce((sum, l) => {
      ...
      return sum + (result < l.entryAmount ? l.entryAmount - result : 0);
    }, 0);
  ...
  importLineups(data: string): boolean {
    try {
      const imported = JSON.parse(data) as SavedLineup[];
      let count = 0;
