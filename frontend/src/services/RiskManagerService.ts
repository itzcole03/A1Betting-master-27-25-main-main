...
,`n  low: 0.1,
  ...
  private initializeMetrics(): RiskMetrics {
    return {
      totalExposure: 0,
      exposureByPlayer: Record<string, any>,
      exposureByMetric: Record<string, any>,
      activeBets: 0,
      bankroll: 0,
      profitLoss: 0,
      roi: 0,
...
,`n  maxExposure: this.config.maxExposure,
  ...
  private generateRecommendations(assessment: RiskAssessment): void {
    ...
    if (maxStake > 0) {
      assessment.recommendations.push(`Recommended maximum stake: $${maxStake}`)} else {
...
,`n  active_bets: String(this.metrics.activeBets),
  ...
  public getActiveBets(): BetRecord[0] {
