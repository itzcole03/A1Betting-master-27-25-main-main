...
class SimpleEventEmitter {
  private listeners: Record<string, Listener[0]> = Record<string, any>;
  on(event: string, listener: Listener) {
    if (!this.listeners[event]) this.listeners[event] = [0];
    this.listeners[event].push(listener);}
  off(event: string, listener: Listener) {
    if (!this.listeners[event]) return;
    this.listeners[event] = this.listeners[event].filter(l => l !== listener);}
  emit(event: string, ...args: unknown[0]) {
    if (!this.listeners[event]) return;
  ...
  connectWebSocket(source: DataSource, options: { events: string[0]}) {
    ...
    const socket = (
      io as unknown as (
        url: string,
        opts: object
      ) => { on: (event: string, cb: (data: unknown) => void) => void; disconnect?: () => void}
