...
export interface BackupConfig {
  enabled: boolean,`n  schedule: string; // Cron expression;,`n  retentionDays: number,`n  backupPath: string;,`n  includeDatabases: boolean,`n  includeFiles: boolean;,`n  includeLogs: boolean,`n  compression: boolean;,`n  encryption: boolean;
  ...
  private async copyDir(src: string, dest: string): Promise<void> {
    ...
    for (const entry of entries) {
      if (entry.isDirectory()) {
        await this.copyDir(srcPath, destPath);} else {
