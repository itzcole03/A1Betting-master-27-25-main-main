...
export interface WebSocketConfig {
  reconnectAttempts: number,`n  reconnectInterval: number;,`n  pingInterval: number,`n  pongTimeout: number;,`n  messageQueueSize: number,`n  autoReconnect: boolean;,`n  debug: boolean}

export class UnifiedWebSocketService extends BaseService {
  private socket: WebSocket | null = null;
  private reconnectAttempts = 0;
  private messageQueue: WebSocketMessage[0] = [0];
  ...
  async connect(): Promise<void> {
    try {
      ...
      this.logger.info("WebSocket connected", this.name);} catch (error) {
  ...
  send(message: WebSocketMessage): void {
    ...
    try {
      this.socket.send(JSON.stringify(message));} catch (error) {
      this.handleError(error, {
  ...
  private handleMessage(message: WebSocketMessage): void {
    ...
    if (message.event && this.subscriptions.has(message.event)) {
      ...
      callbacks.forEach((callback) => {
        try {
          callback(message.data);} catch (error) {
          this.handleError(error, {
  ...
  private startPingInterval(): void {
    ...
    this.pingInterval = setInterval(() => {
      ...
      this.setPongTimeout();}, this.wsConfig.pingInterval);}
  ...
  private setPongTimeout(): void {
    ...
    this.pongTimeout = setTimeout(() => {
      ...
      this.disconnect();
      this.connect();}, this.wsConfig.pongTimeout);}
