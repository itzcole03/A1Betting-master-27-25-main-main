...
export class UnifiedAnalyticsService extends BaseService {
  ...
  async getBettingStats(
    ...
    marketId: string,
    selectionId: string
  ): Promise<{ totalBets: number; wonBets: number; lostBets: number; winRate: number; profitLoss: number; averageOdds: number; averageStake: number }> {
  ...
  async getPerformanceMetrics(
    ...
    try {
      ...
      return {
        ...
        profitLoss,
        averageStake: 0,
        streaks: { current: currentStreak, longest: bestStreak },
        byConfidence: {},
        accuracy
