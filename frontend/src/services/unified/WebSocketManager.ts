...
export class WebSocketManager extends EventEmitter {
  ...
  private constructor() {
    ...
    this.defaultConfig = {
      url: typeof import.meta !== 'undefined' && import.meta.env && import.meta.env.VITE_WS_URL
  ...
  public connect(config: Partial<WebSocketConfig> = Record<string, any>): void {
  ...
  private setupSocketHandlers(
    ...
    socket.onmessage = event => {
      try {
        this.emit('message', { url: config.url, message})} catch (error) {
        this.emit('error', {
          url: config.url,
  ...
  public send(url: string, message: WSMessage): void {
    ...
    if (connection.isConnected) {
      connection.socket.send(JSON.stringify(message));} else {
  ...
  public getConnectionStatus(): Record<string, boolean> {
    const status: Record<string, boolean> = Record<string, any>;
