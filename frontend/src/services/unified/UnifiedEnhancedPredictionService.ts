...
class UnifiedEnhancedPredictionService {
    ...
    try {
      ...
      const response: UnifiedPredictionResponse = {
        ...
        feature_analysis: {
          ...
          feature_interactions: unifiedResult.feature_engineering.information_theory_metrics.mutual_information,
        ...
        performance_metrics: {
          ...
          gpu_utilization: request.use_gpu_acceleration ? additionalAnalysis.gpu_utilization : undefined,
  ...
  private generateRecommendations(
    ...
    return {
      ...
      risk_level: unifiedResult.risk_assessment.portfolio_risk.value_at_risk > 0.2 ? "high" : unifiedResult.risk_assessment.portfolio_risk.value_at_risk > 0.1 ? "medium" : "low",
      suggested_actions: [
        confidence > 0.8 ? "High confidence prediction - consider larger position" : "Lower confidence - reduce position size",
        rigorScore > 90 ? "Strong mathematical foundation - reliable prediction" : "Review model assumptions",
        additionalAnalysis.outlier_detected ? "Outlier detected - exercise caution" : "Normal data pattern detected",
      ],
  ...
  private validatePredictionResults(unifiedResult: any): {
    ...
    error_bounds: Record<string, number>} {
    return {
      output_valid: !isNaN(prediction) && isFinite(prediction) && confidence >= 0 && confidence <= 1,
