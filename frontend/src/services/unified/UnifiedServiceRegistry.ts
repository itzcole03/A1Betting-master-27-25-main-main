...
export class UnifiedServiceRegistry {
  ...
  public getServiceNames(): string[0] {
  ...
  public async initialize(): Promise<void> {
    ...
    for (const service of this.services.values()) {
      try {
        await (service as { initialize?: () => Promise<void>}).initialize?.();} catch (error) {
        this.logger.error(
  ...
  public async cleanup(): Promise<void> {
    ...
    for (const service of this.services.values()) {
      try {
        await (service as { cleanup?: () => Promise<void>}).cleanup?.();} catch (error) {
        this.logger.error(
