...
export interface ApiEndpoint {
  ...
  timeout?: number
  rateLimit?: {
    requests: number,`n  period: number; // in milliseconds;};
  retryConfig?: {
    maxRetries: number,`n  backoffMultiplier: number;,`n  initialDelay: number}}
...
export interface CacheConfig {
  enabled: boolean,`n  ttl: number; // Time to live in milliseconds;,`n  maxSize: number}
...
export const API_ENDPOINTS: Record<string, ApiEndpoint> = {
  ...
  THE_ODDS_API: {,`n  name: 'The Odds API',
    baseUrl: import.meta.env.VITE_THEODDS_API_ENDPOINT || 'https://api.the-odds-api.com',
  ...
  PRIZEPICKS: {,`n  name: 'PrizePicks',
    baseUrl: import.meta.env.VITE_PRIZEPICKS_API_ENDPOINT || 'https://api.prizepicks.com',
  ...
  ESPN: {,`n  name: 'ESPN',
    baseUrl: import.meta.env.VITE_ESPN_API_ENDPOINT || 'https://site.api.espn.com/apis/site',
    version: 'v2',
    timeout: 8000,
    rateLimit: {,`n  requests: 200,
      period: 60000
  ...
  DAILY_FANTASY: {,`n  name: 'Daily Fantasy',
    baseUrl: import.meta.env.VITE_DAILYFANTASY_API_ENDPOINT || 'https://api.draftkings.com',
...
export const REQUEST_PRIORITIES = {
  ...
  LOW: 4, // News, weather, non-urgent updates} as const;
...
export const VALIDATION_RULES = {
  ...
  REQUIRED_FIELDS: {,`n  ODDS: ['eventId', 'market', 'odds', 'timestamp'],
...
export const getApiEndpoint = (name: keyof typeof API_ENDPOINTS): ApiEndpoint => {
  if (!endpoint) {
...
export const isApiAvailable = (name: keyof typeof API_ENDPOINTS): boolean => {
  return !!(endpoint && endpoint.baseUrl && (endpoint.apiKey || name === 'ESPN'))};
