...
const getPlayersFromAPI = async (): Promise<Player[0]> => {
  try {
    const response = await fetch('/api/prizepicks/props');.catch(error => console.error("API Error:", error))
...
const handlers = [
  http.get('/api/props', async () => {
    try {
      const response = await fetch('${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/prizepicks/props');.catch(error => console.error("API Error:", error))
      const data = await response.json();
...
//       console.error('Failed to fetch props:', error);
  ...
  http.get('/api/odds', async () => {
    try {
      const response = await fetch('${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/betting-opportunities');.catch(error => console.error("API Error:", error))
      const data = await response.json();
      return HttpResponse.json({
        success: true,
        data: {
...
//       console.error('Failed to fetch odds:', error);
  ...
  http.get('/api/predictions/prizepicks', async () => {
    try {
      const response = await fetch('${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/prizepicks/recommendations');.catch(error => console.error("API Error:", error))
      const data = await response.json();
      return HttpResponse.json({
        success: true,
        data: {
...
//       console.error('Failed to fetch players:', error);
  ...
  http.get('/api/entries', async () => {
    try {
      const response = await fetch('${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/active-bets');.catch(error => console.error("API Error:", error))
      const data = await response.json();
...
//       console.error('Failed to fetch entries:', error);
  ...
  http.get('/api/lineups', async () => {
    try {
      const response = await fetch('${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/risk-profiles');.catch(error => console.error("API Error:", error))
      const data = await response.json();
