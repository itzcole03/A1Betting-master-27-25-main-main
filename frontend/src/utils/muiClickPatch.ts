      ...
      if (type === 'click' && typeof listener === 'function') {
        const wrappedListener = function (event: Event) {
          try {
            ...
            if (error instanceof TypeError && error.message.includes('onClick is not a function')) {
 ...
 */
export const safeMUIProps = <T extends { onClick?: () => void}>(props: T): T => {
