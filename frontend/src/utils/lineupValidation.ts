...
export function validateLineup(selectedProps: Map<string, any>): LineupValidationResult {
  ...
  const avgConfidence =
    propsArray.reduce((sum, prop) => sum + (prop.confidence || 0), 0) / propsArray.length;
  if (avgConfidence < 60) {
...
export function calculateLineupProbability(selectedProps: Map<string, any>): number {
  // Calculate combined probability (assuming independence)
  return propsArray.reduce((prob, prop) => {
    return prob * confidence}, 1);}
