...
export class BackendStarter {
  ...
  private static async tryStartViaWebSocket(): Promise<{
    ...
    try {
      ...
      throw new Error('WebSocket start method not available');} catch (error) {
   ...
   */
  static getStartupInstructions(): string[0] {
    return [
  ...
  static async checkBackendStatusWithScan(): Promise<BackendStatus> {
    ...
    return {
      isOnline: false,
      error: 'Backend not found on common ports (8000, 8080, 3000, 5000, 8888, 9000)',
      timestamp: new Date()
    }}
}
