...
import { PrizePicksAdapter, PrizePicksData } from '@/adapters/PrizePicksAdapter';
...
export class DataIntegrationHub {
  ...
  private constructor() {
    ...
    if (theOddsApiKey) {
      const theOddsAdapter = new TheOddsAdapter({
        ...
        cacheTimeout: 300000});
      this.registerDataSource(theOddsAdapter);} else {
    ...
    if (sportsRadarApiKey) {
      const sportsRadarAdapter = new SportsRadarAdapter({
        ...
        cacheTimeout: 300000});
      this.registerDataSource(sportsRadarAdapter);} else {
      // console statement removed}
    ...
    if (dailyFantasyApiKey) {
      const dailyFantasyAdapter = new DailyFantasyAdapter({
        ...
        cacheTimeout: 300000});
      this.registerDataSource(dailyFantasyAdapter);} else {
      // console statement removed}
    ...
    if (prizePicksApiKey) {
      ...
      this.registerDataSource(prizePicksAdapter);} else {
      // console statement removed}
  ...
  private async synchronizeAll(): Promise<void> {
    const trace = this.monitor.startTrace('sync-all', 'data.sync', 'Synchronizing all data sources');
  ...
  private updateSourceMetrics(
    ...
    this.monitor.recordMetric('data_source_metrics_updated', 1, {
      source_id: sourceId,
  ...
  private async integrateData(results: Array<{ id: string; data: any; error: Error | null }>): Promise<void> {
    ...
    this.monitor.recordMetric('data_integrated', 1, {
      timestamp: Date.now()
    });
  }
  ...
  private updateIntegratedDataSource(newData: IntegratedData, result: { id: string; data: any; error: Error | null }): void {
    ...
    switch (result.id) {
      ...
      case 'social-sentiment': // Assuming SocialSentimentAdapter.id is 'social-sentiment'
        this.integrateSentiment(newData, result.data as SocialSentimentData[]);
        break;
  ...
  private integratePrizePicksProjections(newData: IntegratedData, prizePicksData: PrizePicksData): void {
    ...
    prizePicksData.projections.forEach(proj => {
      ...
      const existingConfidence = newData.projections[proj.playerId].confidence;
  ...
  private calculateOddsMovement(
    ...
    const previousData = this.dataCache.get(eventId);
    if (!previousData) {
      return { direction: 'stable', magnitude: 0 };
    }

    const previousMarkets = previousData.data.markets;
    const differences = Object.keys(currentMarkets).map(key => {
      const current = currentMarkets[key];
      const previous = previousMarkets[key];
      return previous ? current - previous : 0;
    });

    const magnitude = Math.abs(differences.reduce((a, b) => a + b, 0) / differences.length);
    if (magnitude < 0.05) return { direction: 'stable', magnitude };

    const difference = differences.reduce((a, b) => a + b, 0);
    return {
    ...
    Object.entries(newData.projections).forEach(([playerId, projection]) => {
      Object.entries(projection.stats).forEach(([metric, value]) => {
        if (previousValue !== undefined) {
          ...
          const significance = this.calculateTrendSignificance(change, value);
  ...
  private analyzeSentimentTrends(newData: IntegratedData): void {
    Object.entries(newData.sentiment).forEach(([playerId, sentiment]) => {
      if (previousSentiment) {
        ...
        const significance = this.calculateTrendSignificance(change, sentiment.sentiment.score);
        newData.trends[trendKey] = {
    ...
    Object.entries(newData.odds).forEach(([eventId, odds]) => {
      Object.entries(odds.markets).forEach(([market, price]) => {
        if (previousPrice !== undefined) {
          ...
          const significance = this.calculateTrendSignificance(change, price);
          newData.trends[trendKey] = {
            value: price,
            change,
            significance
  ...
  private analyzeCorrelationTrends(newData: IntegratedData, correlations: DataCorrelation[]): void {
    ...
    for (const correlation of correlations) {
      const correlationValue = this.calculateCorrelation(
        Object.values(newData.projections[correlation.sourceA].stats),
        Object.values(newData.projections[correlation.sourceB].stats)
      );
      newData.trends[trendKey] = {
        value: correlationValue,
        change: 0,
        significance: this.calculateTrendSignificance(correlationValue, 0)
      };
    }
  }
  ...
  private calculateCorrelation(series1: number[], series2: number[]): number {
    ...
    const variance1 = series1.reduce((a, b) => a + Math.pow(b - mean1, 2), 0) / series1.length;
    const variance2 = series2.reduce((a, b) => a + Math.pow(b - mean2, 2), 0) / series2.length;
  ...
  private setupEventListeners(): void {
    ...
    this.eventBus.on('config: updated', (event) => {
      if (event.section === 'dataSources' || event.section === 'all') Record<string, any>
    });
  }
