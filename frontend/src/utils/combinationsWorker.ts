...
interface WorkerMessage {
  type: 'GENERATE_COMBINATIONS',`n  data: {,`n  projections: PrizePicksProjection[0],`n  maxLegs: number}}
...
function generateCombinations(
  ...
  maxLegs: number
): PrizePicksProjection[0][0] {
  const results: PrizePicksProjection[0][0] = [0];

  const batchCount = 0;
  
  const combine = (current: PrizePicksProjection[0], start: number, legsLeft: number) => {
    if (legsLeft === 0) {
