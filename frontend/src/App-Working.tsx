...
const App: React.FC = () => {
  ...
  const getStatusDotColor = () => {
    switch (connectionStatus) {
      case 'connected': return 'bg-green-300';
      case 'connecting': return 'bg-yellow-300';
      case 'disconnected': return 'bg-red-300';
        ...
        <header className="text-center mb-8">
          ...
          {backendStatus && (
            <div className="mt-2 text-sm text-gray-400">
              ...
              Models: {backendStatus.models_loaded || 0} loaded
            </div>
          )}
        </header>
            ...
            {healthData ? (
              <div className="space-y-3">
                <div className="grid grid-cols-2 gap-2 text-sm">
                  <div>Database: <span className={healthData.services.database ? 'text-green-400' : 'text-red-400'}>
                    {healthData.services.database ? '✅ Online' : '❌ Offline'}
                  </span></div>
                  <div>ML Models: <span className={healthData.services.ml_models ? 'text-green-400' : 'text-red-400'}>
                    {healthData.services.ml_models ? '✅ Loaded' : '❌ Error'}
                  </span></div>
                  <div>API: <span className={healthData.services.api ? 'text-green-400' : 'text-red-400'}>
                    {healthData.services.api ? '✅ Active' : '❌ Down'}
                  </span></div>
                  <div>Predictions: <span className={healthData.services.predictions ? 'text-green-400' : 'text-red-400'}>
                    {healthData.services.predictions ? '✅ Ready' : '❌ Unavailable'}
                  </span></div>
                </div>
                <div className="border-t border-gray-600 pt-3 text-sm">
                  <div>Total Predictions: <span className="text-blue-400">{healthData.metrics.total_predictions}</span></div>
                  <div>Model Accuracy: <span className="text-green-400">{(healthData.metrics.model_accuracy * 100).toFixed(1)}%</span></div>
                  <div>Uptime: <span className="text-gray-300">{healthData.metrics.uptime}</span></div>
                </div>
              </div>
            ) : backendStatus ? (
              <div className="space-y-2 text-sm">
                <div>Status: <span className="text-green-400">{backendStatus.status}</span></div>
                <div>Timestamp: <span className="text-gray-300">{backendStatus.timestamp}</span></div>
                <div>Predictions: <span className="text-blue-400">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
          <div className="bg-gray-800/30 border border-gray-700 rounded-lg p-6 text-center">
            ...
            <p className="text-sm text-gray-400">
              {healthData ? `${(healthData.metrics.model_accuracy * 100).toFixed(1)}%` : '96.4%'} accuracy ML models
            </p>
          </div>
