...
//         console.log('✅ Fetched unified data:', Object.keys(unifiedData || {}));
      ...
      if (sport) {
        ...
        opportunitiesData = opportunitiesData.filter(opp => opp.sport?.toLowerCase() === sport.toLowerCase());
      }

      if (propType) {
        propsData = propsData.filter(prop => prop.stat_type?.toLowerCase() === propType.toLowerCase());
      }
