...
const getPlayers = async (): Promise<Player[0]> => {
...
//       searchTerm
  ...
  const positions = useMemo(() => {
    return Array.from(new Set(players.map(p => p.position))).sort();}, [players]);

  const teams = useMemo(() => {
    return Array.from(new Set(players.map(p => p.team))).sort();}, [players]);

  const validateLineup = useCallback((selectedPlayers: Player[0]) => {
