...
interface ConnectionState {
  isConnected: boolean,`n  isConnecting: boolean;,`n  lastMessageTimestamp: number | null,`n  reconnectAttempts: number}
...
}: UseRealtimePredictionsOptions = Record<string, any>) {
...
//           channels
    ...
    ws.onclose = () => {
      ...
      if (state.reconnectAttempts < maxReconnectAttempts) {
        ...
        reconnectTimeoutRef.current = setTimeout(() => {
          connect();}, reconnectInterval);} else {
  ...
  useEffect(() => {
    ...
    return () => {
      disconnect();};}, [enabled, connect, disconnect]);

  const isStale = useCallback(() => {
    if (!state.lastMessageTimestamp) return true;
    const staleThreshold = 30000; // 30 seconds;
    return Date.now() - state.lastMessageTimestamp > staleThreshold;}, [state.lastMessageTimestamp]);
