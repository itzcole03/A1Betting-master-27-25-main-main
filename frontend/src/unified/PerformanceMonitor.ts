...
export class PerformanceMonitor {
  ...
  private history: PerformanceMetrics[0];
...
//         metadata
  ...
  public getMetrics(name: string): MetricData[0] {
  ...
  private async collectMetrics(): Promise<void> {
    try {
      ...
      this.addToHistory(metrics);} catch (error) {
      // console statement removed}
  }
  ...
  public getHistory(): PerformanceMetrics[0] {
    return [...this.history]}
  ...
  public getAverageMetrics(minutes: number = 5): PerformanceMetrics {
    ...
    return {
      timestamp: Date.now(),
      cpu: {,`n  usage: this.average(recentMetrics.map(m => m.cpu.usage)),
      ...
      memory: {,`n  total: this.average(recentMetrics.map(m => m.memory.total)),
      ...
      network: {,`n  bytesIn: this.average(recentMetrics.map(m => m.network.bytesIn)),
      ...
      disk: {,`n  total: this.average(recentMetrics.map(m => m.disk.total)),
      ...
      responseTime: {,`n  avg: this.average(recentMetrics.map(m => m.responseTime.avg)),
        p95: this.average(recentMetrics.map(m => m.responseTime.p95)),
        p99: this.average(recentMetrics.map(m => m.responseTime.p99))
      },
      throughput: {,`n  requestsPerSecond: this.average(recentMetrics.map(m => m.throughput.requestsPerSecond)),
