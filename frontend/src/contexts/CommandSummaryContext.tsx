...
const executeCommand = async (cmd: CommandSummary, updateStatus: (status: string, result?: any) => void) => {
...
export const CommandSummaryProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  ...
  return (
    <CommandSummaryContext.Provider value={{ commands, loading, error, refresh: fetchSummary, queue, addToQueue, removeFromQueue }}>
      {children}
