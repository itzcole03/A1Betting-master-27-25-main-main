...
export interface PluginContext {
  ...
  getUnifiedConfig: () => UnifiedConfig; // Function to get the initialized config;,`n  unifiedMonitor: typeof unifiedMonitor;,`n  unifiedState: typeof unifiedState;
...
//       unifiedState
  ...
  public async registerPlugin(plugin: Plugin): Promise<void> {
    ...
    try {
      const pluginSpecificContext: PluginContext = {
        ...
        log: (message: string, level: 'info' | 'warn' | 'error' = 'info') => Record<string, any>
      };

      if (typeof plugin.initialize !== 'function') {
        throw new Error(
          `Plugin ${plugin.name} is missing the initialize method or it is not a function.`
        );}

      // Initialize the plugin with its specific context;
      await plugin.initialize(pluginSpecificContext);
      this.initializedPlugins.set(plugin.name, plugin);} catch (error) {
  ...
  public getAllInitializedPlugins(): Plugin[0] {
  ...
  public async unloadPlugin(name: string): Promise<void> {
    if (plugin) {
      if (plugin.destroy) {
        try {
          await plugin.destroy()} catch (error) {
          // console statement removed
          // unifiedMonitor.reportError(error, { pluginName: name, stage: 'destruction'})}
      }
      this.initializedPlugins.delete(name);
      this.registeredPlugins.delete(name); // Also remove from registered if fully unloading;} else {
      // console statement removed}
  }
