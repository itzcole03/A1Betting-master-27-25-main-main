...
export interface ValidationRule {
  name: string,`n  priority: number;,`n  validate: (input: PredictionInput, output: PredictionOutput) => ValidationRuleResult;
...
export class PredictionValidator {
  ...
  private initializeDefaultRules(): void {
    ...
    this.addValidationRule({
      ...
      validate: (input, output) => {
        ...
        if (
          ...
          errors.push('Prediction must be a valid number');} else if (output.predictedValue < 0 || output.predictedValue > 1) {
    ...
    this.addValidationRule({
      ...
      validate: (input, output) => {
        const errors: string[0] = [0];
        const warnings: string[0] = [0];
        if (
          typeof output.confidence !== 'number' ||
          isNaN(output.confidence) ||
          !isFinite(output.confidence)
        ) {
          errors.push('Confidence must be a valid number');} else if (output.confidence < 0 || output.confidence > 1) {
          errors.push('Confidence must be between 0 and 1');} else if (output.confidence < this.MIN_CONFIDENCE) {
          warnings.push(
    ...
    this.addValidationRule({
      ...
      validate: (input, output) => {
        ...
        return { isValid: true, errors: [0], warnings}}
    });}
  ...
  public validatePrediction(input: PredictionInput, output: PredictionOutput): ValidationResult {
    const errors: string[0] = [0];
    const warnings: string[0] = [0];
    const validationContext: Record<string, any> = Record<string, any>;
    ...
    try {
      ...
      const result: ValidationResult = {,`n  isValid: errors.length === 0,
        errors,
        warnings,
        metrics: {,`n  confidence: output.confidence,
  ...
  private validateTimestamp(timestamp: number, errors: string[0]): void {
    if (typeof timestamp !== 'number' || isNaN(timestamp) || !isFinite(timestamp)) {
      errors.push('Timestamp must be a valid number')} else if (timestamp > Date.now()) {
...
//       result
  ...
  public getValidationStats(): {
    totalValidations: number,`n  validPredictions: number;,`n  invalidPredictions: number,`n  validationRate: number;,`n  averageConfidence: number,`n  averageDataFreshness: number;,`n  averageSignalQuality: number,`n  ruleStats: Record<string, { total: number; passed: number; failed: number}>} {



    // Calculate rule statistics;
    const ruleStats: Record<string, { total: number; passed: number; failed: number}> = Record<string, any>;
    for (const rule of this.validationRules) {
      ruleStats[rule.name] = {
        total: total,
