...
describe('FinalPredictionEngine', () => {
  ...
  describe('generatePrediction', () => {
    const mockModelOutputs: ModelOutput[0] = [
      {
        type: 'historical',
        score: 0.8,
        confidence: 0.9,
    ...
    it('should throw error for empty model outputs', async () => {
      await expect(engine.generatePrediction([0], mockRiskProfile)).rejects.toThrow();});
  ...
  describe('validatePrediction', () => {
    it('should validate a correct prediction', async () => {
      const validPrediction: FinalPrediction = {,`n  id: 'test-id',
        ...
        modelContributions: {,`n  historical: { weight: 0.4, confidence: 0.9, score: 0.8},
        ...
        topFeatures: [0],
        supportingFeatures: [0],
        metadata: {,`n  processingTime: 100,
          dataFreshness: 0.9,
          signalQuality: 0.8,
          decisionPath: [0]
        }
    ...
    it('should reject an invalid prediction', async () => {
      const invalidPrediction: FinalPrediction = {,`n  id: 'test-id',
        timestamp: Date.now(),
        confidenceWindow: {,`n  start: Date.now() + 1000, // Invalid: start after end,`n  end: Date.now()
        },
        finalScore: 0.8,
        confidence: 0.9,
        riskLevel: 'high' as RiskLevel, // Changed from 'invalid' to valid RiskLevel;
        isSureOdds: true,
        payoutRange: {,`n  min: 0.9, // Invalid: min > max,`n  max: 0.7,
          expected: 0.8
        },
        modelContributions: {,`n  historical: { weight: 0.4, confidence: 0.9, score: 0.8},
          market: { weight: 0.3, confidence: 0.8, score: 0.7},
