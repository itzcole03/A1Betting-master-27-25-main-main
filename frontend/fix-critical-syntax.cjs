...
function getAllTSFiles(dir) {
  ...
  for (const item of items) {
    ...
    } else if (stat.isFile() && (item.endsWith('.ts') || item.endsWith('.tsx')) && stat.size < 2 * 1024 * 1024) {
      files.push(fullPath);
    }
  }
...
function fixFile(filePath) {
  try {
    ...
    content = content.replace(/return\s+\{\s*projections:\s*data\.projections\s*\}\s*\}\s*catch/g, 
