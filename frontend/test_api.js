...
fetch('${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/prizepicks/props').catch(error => console.error("API Error:", error))
  .then(res => res.json())
  .then(data => {
...
fetch('${process.env.REACT_APP_API_URL || "http://localhost:8000"}/api/prizepicks/comprehensive-projections').catch(error => console.error("API Error:", error))
  .then(res => res.json())
  .then(data => {
